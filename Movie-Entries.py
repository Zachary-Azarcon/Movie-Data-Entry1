...
class MovieDatabaseApp:
    def __init__(self, root):
        ...
        rating_frame.grid(row=3, column=1, padx=10, pady=5, columnspan=4, sticky=tk.W)

        for i in range(1, 5):  
            radio_button = tk.Radiobutton(rating_frame, text=str(i), variable=self.rating_var, value=i, bg="#F0F0F0", font=("Helvetica", 12), padx=-15)
        ...
        self.table = ttk.Treeview(self.root, columns=("index", "title", "director", "genres", "rating", "release_year"), style="Custom.Treeview")
        ...
        self.table.grid(row=0, column=2, rowspan=7, padx=10, pady=10, sticky=(tk.N, tk.S, tk.W, tk.E))

        self.scrollbar = ttk.Scrollbar(self.root, orient="vertical", command=self.table.yview)
        self.scrollbar.grid(row=0, column=3, rowspan=7, sticky="ns")
    ...
    def update_table(self):
        try:
            ...
            for i, entry in enumerate(self.entries):
                stars = "★" * min(int(entry["rating"]), 4) + "☆" * (4 - min(int(entry["rating"]), 4))
